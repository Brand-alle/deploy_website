...
class User:
    ...
    def authenticated_user_by_input(cls, user_input):
        ...
        if not existing_user:
            ...
            if not password_valid:
                valid = False
        if not EMAIL_REGEX.match(user_input['log_email']): 
            flash("Invalid email format, registration incomplete!", "login")
            valid = False
        if len(user_input["log_email"]) <= 0 or len(user_input["password"]) <=0:
            flash("All fields are required, Login incomplete!", "login")
            valid = False
        if not valid:
    ...
    def is_valid(cls, user):
        ...
        if not any(char.isdigit() for char in user["password"]):
            ...
            is_valid = False
        if "terms" not in user:
            flash("Terms must be accepted before creating an account", "registration")
            is_valid = False
        return is_valid